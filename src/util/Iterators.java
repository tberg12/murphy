...
public class Iterators
{
	...
	public static <S, T> Iterable<T> transform(final Iterable<S> input, final Method<S, T> transform) {
		return new Iterable<T>()
		{

			@Override
			public Iterator<T> iterator() {
				return new TransformingIterator<S, T>(input.iterator(), transform);
			}
		};
	}
	...
	public static <S, T> Iterator<Pair<S, T>> zip(final Iterator<S> s, final Iterator<T> t) {
		return new Iterator<Pair<S, T>>()
		{
			...
			public Pair<S, T> next() {
				return Pair.makePair(s.next(), t.next());
			}
	...
	public static <T> Iterable<T> maxLengthIterable(final Iterable<T> base, final int max) {
		return new Iterable<T>()
		{

			@Override
			public Iterator<T> iterator() {
				return maxLengthIterator(base.iterator(), max);
			}
	...
	public static <T, I extends Iterable<T>> Iterable<T> flatMap(final Iterable<I> base) {
		return new Iterable<T>()
		{

			@Override
			public Iterator<T> iterator() {
				return new FlatteningIterator<T>(new TransformingIterator<I, Iterator<T>>(base.iterator(), new Method<I, Iterator<T>>()
				{

					@Override
					public Iterator<T> call(I obj) {
						return obj.iterator();
					}
				}));
			}
	...
	public static class FlatteningIterator<T> implements Iterator<T>
	{
		...
		Iterator<? extends Iterator<T>> keys;

		public FlatteningIterator(Iterator<? extends Iterator<T>> keys) {
			this.keys = keys;
			current = getNextIterator();
		}
	...
	public static class FilteredIterator<T> implements Iterator<T>
	{
		...
		public FilteredIterator(Filter<? super T> filter, Iterator<T> base) {
			super();
			this.filter = filter;
			this.base = base;
			loadNext();
	...
	public static <T> Iterable<T> filter(final Iterable<T> iterator, final Filter<? super T> filter) {
		return new Iterable<T>()
		{

			@Override
			public Iterator<T> iterator() {
				return new FilteredIterator<T>(filter, iterator.iterator());
			}
		};
	}
	...
	public static <T> Iterator<T> concat(Iterable<Iterator<T>> args) {
		...
		return new IteratorIterator<T>(args.iterator(), factory);
	}
