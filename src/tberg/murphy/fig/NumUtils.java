...
public class NumUtils
{
	...
	public static void assertEquals(double x, double y, double tol) {
		assert approxEquals(x, y, tol) : x + " != " + y;
	}

	public static boolean approxEquals(double x, double y, double tol) {
		return Math.abs(x - y) < tol;
	}

	public static boolean approxEquals(double x, double y) {
		return approxEquals(x, y, 1e-10);
	}
	...
	// Return log(gamma(xx))
	private static double[] logGammaCoeff = { 76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155, 0.1208650973866179e-2,
			-0.5395239384953e-5 };
	...
	public static double digamma(double x) {
		...
		f = 1 / (x * x);
		t = f
			* (-1 / 12.0 + f * (1 / 120.0 + f * (-1 / 252.0 + f * (1 / 240.0 + f * (-1 / 132.0 + f * (691 / 32760.0 + f * (-1 / 12.0 + f * 3617 / 8160.0)))))));
	...
	public static boolean normalizeLeaveZero(double[] data) {
		double sum = 0;
		for (double x : data)
			sum += x;
		if (sum == 0) {
	...
	public static boolean relativeApproxEquals(double a, double b, double tol) {
