...
class OptInfo
{
	...
	public String getValueString() {
		...
		if (o instanceof Pair) return ((Pair) o).getFirst() + "," + ((Pair) o).getSecond();
	...
	public String toString() {
		...
		String s = String.format("%-30s <%5s> : %s [%s] {%s}", fullName(), typeStr(), gloss, valueStr, StrUtils.join(addPound(tags), ","));
		String t = getEnumStr();
		if (!t.equals("")) s += " " + t;
		return s;
	}

	private String[] addPound(String[] tags) {
		String[] n = new String[tags.length];
...
public class OptionsParser
{
	...
	public OptionsParser doRegister(String group, Object o) {
		...
		for (Method method : classOf(o).getMethods()) {
			OptionSet ann = method.getAnnotation(OptionSet.class);
			if (ann == null) continue;
			try {
	...
	public OptionsParser doRegisterAll(Object[] objects) {
		...
		for (Object o : objects) {
			...
			if (o instanceof String)
				name = (String) o;
			else if (o instanceof Class<?> && ((Class<?>) o).isAnnotation()) {
				additionalOptionAnnotations.add((Class<? extends Annotation>) o);
			} else {
				if (name == null) {
	...
	private List<OptInfo> matchOpt(ArrayList<OptInfo> options, String s, boolean allowMultipleMatches) {
		...
		if (allowMultipleMatches)
			...
			// Enforce one match
			if (completeMatches.size() == 1) return ListUtils.newList(completeMatches.get(0));
			if (completeMatches.size() == 0 && partialMatches.size() == 1) return ListUtils.newList(partialMatches.get(0));
	...
	private static void printHelp(List<OptInfo> options, Set<String> tags) {
		...
		for (OptInfo opt : options) {
			...
			if (tags.isEmpty())
				...
				for (String optTag : opt.tags) {
					if (tags.contains(optTag)) {
						shouldPrint = true;
						break;
					}
				}
			}
			if (shouldPrint) opt.print();

		}
	}

	public void printHelp() {
		printHelp(options, new HashSet<String>());
	}
		...
		for (String group : objects.keySet()) {
			...
			for (Field field : classOf(obj).getFields()) {
				...
				if (ann != null) {
					...
					for (Class<? extends Annotation> additionalOptionsAnn : additionalOptionAnnotations) {
						if (field.isAnnotationPresent(additionalOptionsAnn)) {
							OptInfo opt = new OptInfo();
							Annotation annotation = field.getAnnotation(additionalOptionsAnn);
							opt.group = group;
							try {
								String annName = (String) annotation.getClass().getMethod("name").invoke(annotation);
								String annGloss = (String) annotation.getClass().getMethod("gloss").invoke(annotation);
								String[] tags = (String[]) annotation.getClass().getMethod("tags").invoke(annotation);
								String condReq = (String) annotation.getClass().getMethod("condReq").invoke(annotation);
								boolean req = (Boolean) annotation.getClass().getMethod("required").invoke(annotation);
								opt.name = annName.equals("") ? field.getName() : annName;
								opt.gloss = annGloss;
								opt.tags = tags;
								opt.condReq = condReq;
								opt.required = req;
								...
								throw new RuntimeException("Problem with additional options type " + additionalOptionsAnn + " : ", e);
			...
			if (info == null) // Shouldn't happen, but if it does, the user will be notified
				...
				if (info.getValue() instanceof ArrayList) { // For an array, suffices if just one element matches
					for (Object x : (ArrayList) info.getValue())
						if (x.toString().matches(value)) return o.toString() + ", " + o.condReq + " holds";
				} else {
					if (info.getValueString().matches(value)) return o.toString() + ", " + o.condReq + " holds";
				}
			}
		}
		return null;
	...
	private ArrayList<Class<? extends Annotation>> additionalOptionAnnotations = new ArrayList<Class<? extends Annotation>>();
