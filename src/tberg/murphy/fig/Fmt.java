...
public class Fmt {
	...
	public static String bytesToString(long b) {
		...
		if (gb >= 1) return gb >= 10 ? (int) gb + "G" : NumUtils.round(gb, 1) + "G";
		double mb = (double) b / (1024 * 1024);
		if (mb >= 1) return mb >= 10 ? (int) mb + "M" : NumUtils.round(mb, 1) + "M";
		double kb = (double) b / (1024);
		if (kb >= 1) return kb >= 10 ? (int) kb + "K" : NumUtils.round(kb, 1) + "K";
		return b + "";
	}
	...
	public static String ratio(int hits, int checks) {
		return hits + "/" + checks +" (" + Fmt.D(100.0 * hits/checks) + "%)";
	}
}
