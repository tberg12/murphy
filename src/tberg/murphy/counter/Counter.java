 ...
 * @author Dan Klein
 */
public class Counter<E> implements Serializable, CounterInterface<E>
{
	...
	public double incrementCount(E key, double increment) {
		if (increment == 0.0) return getCount(key);
	...
	public <T extends E> void incrementAll(CounterInterface<T> counter) {
		incrementAll(counter, 1.0);
	}

	public <T extends E> void incrementAll(CounterInterface<T> counter, double scale) {
		for (Entry<T, Double> entry : counter.entries()) {
	...
	public double dotProduct(CounterInterface<E> other) {
		double sum = 0.0;
		CounterInterface<E> o1 = null;
		CounterInterface<E> o2 = null;
		if (size() < other.size()) {
			o1 = this;
			o2 = other;
		} else {
		...
		for (Map.Entry<E, Double> entry : o1.entries()) {
			final double value = entry.getValue();
			if (value == 0.0) continue;
			final double otherCount = o2.getCount(entry.getKey());
	...
	{
		public int compare(Entry<E, Double> e1, Entry<E, Double> e2) {
	...
	public Iterable<Entry<E, Double>> entries() {
		return entrySet();
	}
