...
//	}
	...
	public Counter<V> getCounterPossiblyNull(K key) {
		return counterMap.get(key);
	}

	public Counter<V> getCounterNoInsert(K key) {
		Counter<V> counter = counterMap.get(key);
		return counter == null ? new Counter<V>() : counter;
	}
  ...
  public String toString(Collection<String> keyFilter) {
	...
	public boolean containsKey(K s1, V s2)
	{
		Counter<V> valueCounter = counterMap.get(s1);
		if (valueCounter == null) { return false; }
		return valueCounter.containsKey(s2);
	}
	
	public CounterMap<K, V> copy()
	{
		CounterMap<K, V> c = new CounterMap<K, V>();
		c.incrementAll(this);
		return c;
	}
	...
	{
		for (Iterator<Entry<K, Counter<V>>> iterator = entrySet().iterator(); iterator.hasNext();)
		{
			Entry<K, Counter<V>> entry = iterator.next();
			final Counter<V> counter = entry.getValue();
			counter.pruneKeysBelowThreshold(d);
			if (counter.isEmpty()) iterator.remove();
		}
	}

	public void removeKey(K k, V v)
	{
		Counter<V> valueCounter = counterMap.get(k);
		if (valueCounter == null) return;
		valueCounter.removeKey(v);
		if (valueCounter.isEmpty()) counterMap.remove(k);
	}
